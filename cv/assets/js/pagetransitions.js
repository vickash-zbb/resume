	...
	function init() {
	    ...
	    * Functions 
	    ...
	    function open_sec_on_load() {
	    	...
	    	if ( $(".front_home").length ) { window__hash('#home', false, false); }
	    ...
	    function window__hash(elem, delay = false, changeHash = true){
	    	$('#menu a[href="'+elem+'"]').addClass('active_item').parents("li").siblings().find('a').removeClass('active_item'); 
	    	if (changeHash) {
		    	if (delay){
		    		hashTimeout = setTimeout(function(){ window.location.hash =  elem; },600);
		    	}else{
		...
		$(window).on('hashchange', function() {
			...
			if (hash){
				...
				$('#menu a[href="#home"]').trigger("click");
			}*/
